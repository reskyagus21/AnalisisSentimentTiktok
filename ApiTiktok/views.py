...
def scrape_tiktok_comments(video_url, max_scroll=15, scroll_delay=2):
    try:
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=False)
            context = browser.new_context()
            page = context.new_page()
            print(f"[INFO] Membuka: {video_url}")
            ...
            try:
                page.wait_for_selector('p[data-e2e="comment-level-1"]', timeout=10000)
            except Exception as e:
                logger.error(f"Komentar tidak muncul : {str(e)}")
                browser.close()
                return []
            comment_elements = page.query_selector_all('p[data-e2e="comment-level-1"]')
            comments = [el.inner_text().strip() for el in comment_elements if el.inner_text().strip()]
            browser.close()
            return comments
    except Exception as e:
        logger.error(f"Gagal mengambil komentar : {str(e)}")
        return[], f"Gagal mengakses video:{str(e)}"
...
def tokenize_and_count(texts):
    all_words = []
    stopwords = {'ini', 'di', 'yang', 'dan', 'apa', 'siapa', 'ke', 'dia', 'saya', 'atau', 'itu', 'bisa', 'ada', 'adalah'}
    for text in texts:
    ...
    if request.method == 'POST':
        ...
        if action == 'analyze':
            ...
            if len(new_csv_data) < len(csv_data):
                ...
                Dokument.objects.filter(uuid=row_id).delete()  # Cascades to AnalysisResult
